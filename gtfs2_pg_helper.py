...
def get_gtfs2_pg(hass, path, data, update=False):
...
def get_local_stops_next_departures_old(self):
...
#    radius = self._data.get("radius", DEFAULT_LOCAL_STOP_RADIUS) / 1111111
    radius = ( 360 * self._data.get("radius", DEFAULT_LOCAL_STOP_RADIUS) ) / ( 40000 * 1000 )
    ...
    _LOGGER.info("get_local_stops_next_departures_old: In %dms Processed ", elapsed / datetime.timedelta(milliseconds=1))
...
def get_local_stops_next_departures_new(self):
    ...
    sql_query = f"""
        SELECT stop.stop_id, stop.stop_name,stop.stop_lat as latitude, stop.stop_lon as longitude 
            FROM stops stop
            where {latitude_longitude_where_clause}
    """  # noqa: S608
    result = schedule.engine.connect().execute(
        text(sql_query),
        {},
    )        
    # filter stops rows on real distance, and build a list of id
    sql_rows_stops = []
    count_sql_rows = 0
    id_list = ""
    defined_radius = self._data.get("radius", DEFAULT_LOCAL_STOP_RADIUS)
    for row_cursor in result:
        row = row_cursor._asdict()
        coords_1 = (latitude ,longitude)
        coords_2 = (row['latitude'], row['longitude'])
        distance_meters = geopy.distance.geodesic(coords_1, coords_2).meters 
        if distance_meters > defined_radius :
            _LOGGER.info("Distance filter REJECT ( %d > %d meters): %s",distance_meters, defined_radius, row)
        else:
            #_LOGGER.info("Row added ( %d <  %d meters): %s",distance_meters, defined_radius, row)
            count_sql_rows = count_sql_rows + 1    
            sql_rows_stops.append(row)
            if id_list == "":
                id_list =  "'"+ row["stop_id"] + "'"
            else:
    ...
    #
    id_list = "(" + id_list + ")"
    #_LOGGER.info("stop_id within %d metres = : %s", defined_radius,  id_list)
    ...
    sql_query = f"""
        SELECT * FROM (
        SELECT st.stop_id as stop_id, trip.trip_id, trip.trip_headsign, trip.direction_id, time(st.departure_time) as departure_time,
               route.route_long_name,route.route_short_name,route.route_type,
               calendar.{now.strftime("%A").lower()} AS today,
               {tomorrow_select}
               calendar.start_date AS start_date,
        ...
        order by tomorrow, departure_time
        """  # noqa: S608
    result = schedule.engine.connect().execute(
        text(sql_query),
        {
    ...
    # limit number of rows to process. ( hopefully, lines are sorted by time )
    max_rows = 50000
    ...
    if max_rows_reached :    
        _LOGGER.error("MAX ROWS %d reached !!!, result cpt_rows truncated ", max_rows, cpt_rows )



    count_row_stop=0
    for row in sql_rows_stops:
        count_row_stop = count_row_stop + 1

    count_sql_rows_result=0
    for row in sql_rows_result:
        count_sql_rows_result = count_sql_rows_result + 1

    departures_returned = 0

    for row_stop in sql_rows_stops:
        timetable = []
        for row in sql_rows_result:
            if row['stop_id'] == row_stop["stop_id"]:
                ...
                if (case1 and case2):
                    _LOGGER.debug("CASE 1 is True , and CASE 2 is True => Will be processed twice!!! ", )

                if (not case1) and (not case2):
                    _LOGGER.debug("CASE 1 is False , and CASE 2 is False => Will not be procesed at all")
                ...
                if row["today"] == 1 or (row["today_cd"] == 1 and row["start_date"] == row["calendar_date"]):
                    ...
                    if depart_time_corrected > now.replace(tzinfo=timezone): 
                        ...
                        timetable.append({"departure": row["departure_time"], "departure_realtime": departure_rt, "delay_realtime": delay_rt, "date": now_date, "stop_name": row_stop['stop_name'], "route": row["route_short_name"], "route_long": row["route_long_name"], "headsign": row["trip_headsign"], "trip_id": row["trip_id"], "direction_id": row["direction_id"], "icon": self._icon})
                    else:
                        _LOGGER.debug("Departure time dropped: %s, before now: %s", depart_time_corrected, now.replace(tzinfo=timezone))
                        timetable.append({"departure": row["departure_time"], "departure_realtime": departure_rt, "delay_realtime": delay_rt, "date": now_date, "stop_name": row_stop['stop_name'], "route": row["route_short_name"], "route_long": row["route_long_name"], "headsign": row["trip_headsign"], "trip_id": row["trip_id"], "direction_id": row["direction_id"], "icon": "deleted"})
                ...
                if (
                    ...
                    _LOGGER.debug("Tomorrow adding row_tomorrow: %s, row_today: %s, now_time: %s, departure_time: %s", row["tomorrow"],row["today"],now_time_hist_corrected,row["departure_time"])
                    departures_returned = departures_returned + 1
                    timetable.append({"departure": row["departure_time"], "departure_realtime": "tomorrow", "delay_realtime": "tomorrow", "date": tomorrow_date, "stop_name": row_stop['stop_name'], "route": row["route_short_name"], "route_long": row["route_long_name"], "headsign": row["trip_headsign"], "trip_id": row["trip_id"], "direction_id": row["direction_id"], "icon": self._icon})
            #endif row['stop_id'] == row_stop["stop_id"]:


        coords_1 = (latitude ,longitude)
        coords_2 = (row_stop['latitude'], row_stop['longitude'])
        distance_meters = geopy.distance.geodesic(coords_1, coords_2).meters 

        entry = {"stop_id": row_stop['stop_id'], "distance": distance_meters, "stop_name": row_stop['stop_name'], "latitude": row_stop['latitude'], "longitude": row_stop['longitude'], "departure": timetable, "offset": offset}
        local_stops_list.append(entry)
    ...
    _LOGGER.info("get_local_stops_next_departures_new: In %dms Processed %d stops %d departures. Returned %d departures:, Returned %d bytes", elapsed / datetime.timedelta(milliseconds=1), count_row_stop, count_sql_rows_result, departures_returned, size_returned )
    return data_returned




def get_local_stops_next_departures(p_self):

    returned_1 =  get_local_stops_next_departures_old(p_self)
    returned_2 =  get_local_stops_next_departures_new(p_self)
#    
#    if returned_1 != returned_2:
#        _LOGGER.warning("get_local_stops_next_departures:  NEW result <> OLD result")
